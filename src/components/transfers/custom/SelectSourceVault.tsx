...
export default function SelectSourceVault({ label, name, vaults, ...textFieldProps }: selectProps) {
  ...
  const [filtered, setFiltered] = React.useState<VaultResponse[]>(
    (vaults || []).filter((v) => v.campaignId === values.sourceCampaignId),
  )

  React.useEffect(() => {
    const selected = (vaults || []).filter((v) => v.campaignId === values.sourceCampaignId)
    setFiltered(selected)
    if (selected.length === 0) {
      setFieldValue(name, '')
    }
  }, [name, values.sourceCampaignId, values.sourceVaultId])
