...
export function NewsAdminPage({slug, isAdmin} : Props) {
  ...
  const columns: GridColumns = [
    ...
    },
    {
      ...
      headerClassName: classes.gridColumn,
      renderCell: (params: GridRenderCellParams): React.ReactNode => {
        return params.row.editedAt 
        ? formatDateString(params.row.editedAt, i18n.language)
        : ''
      },
      minWidth: 200,
      flex: 1,
    ...
    <Box>
      ...
      <Tooltip title={cantEdit 
        ...
        : null
          ...
          <span>
        <IconButton size="small" color="primary" onClick={() => setDeleteId(params.row.id)} disabled={cantEdit}>
          <DeleteOutlinedIcon />
        </IconButton>
          </span>
        </Tooltip>        
    ...
    <Grid item>
        ...
        <Link 
        href={{ pathname: routes.campaigns.news.create(slug), query:{campaignTitle: data.title, campaignId: data.id}}}>
