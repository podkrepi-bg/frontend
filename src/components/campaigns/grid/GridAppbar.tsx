  ...
  return (
    <Toolbar
      ...
      <Box sx={{ height: '64px', display: 'flex', alignItems: 'start', pt: 1 }}>
        <Typography>{t('campaigns:all')}</Typography>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'flex-end', pb: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Tooltip title={t('campaigns:cta:deleteSelected') || ''}>
            <DeleteIcon onClick={deleteHandler} sx={iconStyles} fontSize="medium" color="action" />
          </Tooltip>
          <Tooltip title={t('campaigns:cta:add') || ''}>
            <AddIcon
              sx={addIconStyles}
              fontSize="large"
              onClick={() => {
                router.push(routes.admin.campaigns.create)
              }}
            />
          </Tooltip>
        </Box>
