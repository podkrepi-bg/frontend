...
export default function SearchByPhone({ initialValues = defaults }: SearchFormProps) {
  ...
  const onSubmit = async (values: SearchFormData, { resetForm }: FormikHelpers<SearchFormData>) => {
    try {
      setRes([])
      const data = await axios.get('http://localhost:5010/api/city/search/name/' + values.keyword)
      setRes(data.data)
      resetForm()
    } catch (error) {
      console.error(error)
  ...
  return (
    <BootcampersLayout>
      <Grid container direction="column" component="section" style={{ marginLeft: '4%' }}>
        ...
        {res.length > 0 ? (
          <GenericGrid data={res}></GenericGrid>
        ) : (
          <Typography style={{ textAlign: 'center', fontSize: '20px', marginRight: '4%' }}>
            No results
          </Typography>
        )}
      </Grid>
