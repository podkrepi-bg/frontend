...
function DetailsModal() {
  ...
  const onYesButtonClick = async (id: string) => {
    try {
      await axios.delete(`http://localhost:5010/api/campaign-types/remove/${id}`)
      DeleteModalStore.clear()
      AlertStore.show('Successfully removed campaign type', 'success')
      query.refetch()
    } catch (e) {
            ...
            <DialogContent dividers>
              <Grid item xs={12}>
                <Typography variant="h5" style={{ textAlign: 'center' }} component="h2">
                  Are you sure you want to remove this campaign type?
                </Typography>
