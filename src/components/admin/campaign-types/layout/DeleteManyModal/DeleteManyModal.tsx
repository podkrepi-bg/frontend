...
function DetailsModal() {
  ...
  const onYesButtonClick = async (ids: string[]) => {
    try {
      ids.map((x) => {
        axios.delete(`http://localhost:5010/api/campaign-types/remove/${x}`)
      })
      DeleteManyModalStore.clear()
      AlertStore.show('Successfully removed campaign types', 'success')
      query.refetch()
    } catch (e) {
            ...
            <DialogContent dividers>
              <Grid item xs={12}>
                <Typography variant="h5" style={{ textAlign: 'center' }} component="h2">
                  Are you sure you want to remove these campaign types?
                </Typography>
