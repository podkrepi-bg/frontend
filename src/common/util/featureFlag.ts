...
import { isAdmin, SessionRoles, RealmRole, ResourceRole, hasRealmRole, hasResourceRole } from './roles'
...
export const featureEnabledForSession = (
  flagName: Features,
  ...
  const hasRequiredRealm =
    !gate.realmRoles?.length ||
    gate.realmRoles.some((role) => hasRealmRole(sessionRoles, role))
