...
export class BasePage {
     ...
     */
    async waitForElementToBeReadyByLocator(elementLocator: Locator, timeoutParam: number = 10000): Promise<void> {
     ...
     * @param {number} timeoutParam - the default value is 10sec
     */
    async isElementVisibleBySelectorWithTimeout(elementSelector: string, options: any = null, timeoutParam: number = 10000): Promise<boolean> {
        let elementLocator: Locator;
        if (options) {
     ...
     * @param {number} timeoutParam - the default value is 10sec
     */
    async isElementVisibleByLocatorWithTimeout(elementLocator: Locator, timeoutParam: number = 10000): Promise<boolean> {
        try {
     ...
     * @param {any} options
     */
    async getTextOfElementBySelector(elementSelector: string, options?: any): Promise<string | null> {
     ...
     * @param {any} options
     */
    async getAttributeBySelector(elementSelector: string, attributeName: string, options?: any): Promise<string | null> {
        if (options) {
     ...
     * @param {number} timeoutParam
     */
    async getAttributeByLocator(elementLocator: Locator, attributeName: string, timeoutParam: number): Promise<string | null> {
        await this.waitForElementToBePresentedByLocator(elementLocator);
     ...
     */
    async setInputFieldBySelector(elementSelector: string, inputValueToFill: string | number, clearBeforeInput: boolean = true, slowTyping: boolean = false, pressEnterKey: boolean = false): Promise<void> {
     ...
     * @param {boolean} pressEnterKey - the default value is 'false'
     */
    async setInputFieldByLocator(elementLocator: Locator, inputValueToFill: string | number, clearBeforeInput: boolean = true, pressEnterKey: boolean = false): Promise<void> {
        await this.waitForElementToBePresentedByLocator(elementLocator);
        if (clearBeforeInput) {
     ...
     */
    async selectCheckboxSelectorByLabelText(elementSelector: string, labelText: Array<string>): Promise<void> {
        await this.waitForElementToBePresentedBySelector(elementSelector);
        for (const item of labelText) {
            const checkboxElementLocator = this.page.locator(elementSelector, {hasText: item}).locator("input[type='checkbox']");
     ...
     * @param {Array<string>} labelText
     */
    async selectRadioButtonSelectorByLabelText(elementSelector: string, labelText: Array<string>): Promise<void> {
        await this.waitForElementToBePresentedBySelector(elementSelector);
        for (const item of labelText) {
            // Here we use Xpath because of the special symbol "&nbsp" in the English version
            const radioCheckboxElementLocator = this.page.locator("(//input[@type='radio']/ancestor::label/p[contains(text(),'" + item + "')])[1]");
     ...
     * @param {Array<string>} labelText
     */
    async deselectCheckboxSelectorByLabelText(elementSelector: string, labelTextArray: Array<string>): Promise<void> {
        await this.waitForElementToBePresentedBySelector(elementSelector);
     ...
     * @param {Array<string>} labelText
     */
     async isCheckboxSelectorCheckedByLabelText(elementSelector: string, labelText: string): Promise<boolean> {
        await this.waitForElementToBePresentedBySelector(elementSelector);
    ...
    async isCheckboxCheckedByLabelText(labelText: string): Promise<boolean> {
        await this.scrollToElementCenterBySelector(this.checkboxLabelSelector, labelText);
     ...
     */
    // TODO Add poll
    async checkPageUrlByRegExp(urlRegExpAsString: string, timeoutParam: number = 10000): Promise<void> {
        await this.page.waitForTimeout(1000);
        await expect(this.page, "The URL is not correct!").toHaveURL(new RegExp(urlRegExpAsString), {timeout: timeoutParam});
    }
    ...
    async isStepActiveByLabelText(roleText: string): Promise<boolean> {
        return this.isElementVisibleBySelectorWithTimeout(this.activeHorizontalStepSelector, {hasText: roleText});
    }
     ...
     */
    async selectDropdownOptionValue(dropdownSelector: string, dropdownOptionStringOrArray: string | string[]): Promise<void> {
        await this.waitForElementToBeReadyBySelector(dropdownSelector, {timeout: 2000});
