...
test.describe.serial('Support page - Join us as member - BG language version', async () => {
  ...
  const campaignBenefactorText = bgLocalizationSupport.steps['addition-questions'].benefactor.campaignBenefactor;
  ...
  const projectManagerText = bgLocalizationSupport.steps['addition-questions'].volunteer.projectManager;
  ...
  test('"Role" step works correctly', async () => {
    ...
    expect.soft(await supportPage.isAdditionalQuestionsStepActive(), "Additional Questions step is not active.").toBeTruthy();
    expect(await supportPage.isCheckboxCheckedByLabelText(memberText), "Expected checkbox 'Member' is not checked.").toBeTruthy();
  });

  test('"Additional questions" step works correctly', async () => {
    await supportPage.selectCheckboxByLabelText([campaignBenefactorText, npoPartnerText, companySponsorText,
      projectManagerText, backEndText, frontendText]);
    await supportPage.clickSubmitButton();
    expect.soft(await supportPage.isAdditionalQuestionsStepActive(), "Additional Questions step is active, but should not be.").toBeFalsy();
    await supportPage.clickBackButton();
    expect.soft(await supportPage.isCheckboxCheckedByLabelText(campaignBenefactorText), `${campaignBenefactorText} is not checked`).toBeTruthy();
    ...
    expect.soft(await supportPage.isCheckboxCheckedByLabelText(projectManagerText), `${projectManagerText} is not checked`).toBeTruthy();
    expect.soft(await supportPage.isCheckboxCheckedByLabelText(backEndText), `${backEndText} is not checked`).toBeTruthy();
    expect(await supportPage.isCheckboxCheckedByLabelText(frontendText), `${frontendText} is not checked`).toBeTruthy();
  });

  test('"Keep in touch" step works correctly', async () => {
    await supportPage.clickSubmitButton();
    await supportPage.fillContactForm([supportPageVolutneerTestData.firstName, supportPageVolutneerTestData.lastName,
      supportPageVolutneerTestData.email, supportPageVolutneerTestData.phone, supportPageVolutneerTestData.comment]);
    expect(await supportPage.isAdditionalQuestionsStepActive(), "Additional Questions step is active, but should not be.").toBeFalsy();
  });

  test('"Thank you" step works correctly', async () => {
    expect.soft(await supportPage.isContactDataStepActive(), "Contact Data step is active, but should not be.").toBeFalsy();
    expect.soft(await supportPage.isParticipationStepActive(), "Participation step is Not active.").toBeTruthy();
    expect(await supportPage.isThankYouSupportH4HeadingVisible(), "Thank you greeting is not visible.").toBeTruthy();
  });
});
