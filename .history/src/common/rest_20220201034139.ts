...
import { BootcampResponse, BootcampInput, EditBootcampProp } from 'gql/bootcamps'
...
export const createBootcamp: MutationFunction<AxiosResponse<BootcampResponse>, BootcampInput> =
  ...
  // export const viewBootcamp: MutationFunction<AxiosResponse<BootcampResponse>, string> = async (
  //   id: any,
  // ) => {
  //   return await axios.get<any, AxiosResponse<BootcampResponse>>(
  //     endpoints.bootcamp.viewBootcamp(id).url,
  //   )
  // }

  export const viewBootcamp = async (data: string) => {
    return await (
      await axios.get(endpoints.bootcamp.viewBootcamp(data).url)
    ).data
  }

  export const editBootcamp: MutationFunction<
    AxiosResponse<BootcampResponse>,
    EditBootcampProp
  > = async ({ id, data }: EditBootcampProp) => {
    return await axios.patch<BootcampResponse, AxiosResponse<BootcampResponse>>(
      endpoints.bootcamp.editBootcamp(id).url,
      data,
    )
  }
