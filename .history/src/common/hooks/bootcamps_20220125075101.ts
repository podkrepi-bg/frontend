...
export async function useViewBootcamp(id: number) {
  return useQuery<{ bootcamp: BootcampResponse }>(endpoints.bootcamp.view(id).url)
}

export async function useDeleteBootcamp(id: number) {
  return useQuery<{ bootcamp: BootcampResponse }>(endpoints.bootcamp.deleteBootcamp(id).url)
}
