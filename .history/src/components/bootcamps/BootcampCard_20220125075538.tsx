...
import { deleteBootcamp, useDeleteBootcamp } from 'common/hooks/bootcamps'
...
export default function BootcampCard({ bootcamp }: Props) {
  ...
  const handler = async () => {
    return useDeleteBootcamp(bootcamp.id)
  }
  return (
        ...
        <CardContent>
          ...
          <Box mx={2} mb={2}>
            <LinkButton
              onClick={handler}
              fullWidth
              href={routes.bootcamps.deleteBootcamp(bootcamp.id)}
              variant="contained"
              color="secondary"
